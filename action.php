...
// Fetch selected category or brand
...
if (isset($_POST["count_item"])) {
    $sql = isset($_SESSION["uid"]) ?
        "SELECT COUNT(*) AS count_item FROM cart WHERE user_id = $_SESSION[uid]" :
        "SELECT COUNT(*) AS count_item FROM cart WHERE ip_add = '$ip_add' AND user_id < 0";
    $query = mysqli_query($con, $sql);
    $row = mysqli_fetch_array($query);
    echo $row["count_item"];
    exit();
    ...
    if (isset($_POST["getCartItem"])) {
        if (mysqli_num_rows($query) > 0) {
            ...
            while ($row = mysqli_fetch_array($query)) {
                ...
                $cart_item_id = $row["id"];
                $qty = $row["qty"];
                $total_price += $product_price * $qty; // Multiply price by quantity
    ...
    if (isset($_POST["checkOutDetails"])) {
        if (mysqli_num_rows($query) > 0) {
            ...
            if (!isset($_SESSION["uid"])) {
                ...
                echo "<input type='hidden' name='return' value='http://localhost/huster-store/huster-store/payment_success.php'/>
